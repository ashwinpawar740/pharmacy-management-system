...
void manageAccount()
{
	...
	printf("\n\xB2 Admin information updated successfully\n");
	system("PAUSE");
	system("CLS");
	adminAccess();
}
	...
	else
	{
		...
		while (fread(&md, sizeof(md), 1, searchId))
		{
			if (stricmp(md.medicineID, seaMedId) == 0)
			{
				...
				printf("\nMedicine price=\t\t\tRs. %0.2f", md.price);
...
void viewAll()
{
	...
	{
		printf("Medicine ID\tMedicine Name\tPrice (Rs.)\tIn-Stock\tExpiry Date");
		printf("\n_________________________________________________________________________________\n");
